...
from torchtext_legacy.data import BucketIterator, Dataset, Field, TabularDataset
...
class NLIDataset(TabularDataset):
    urls = []
    dirname = ""
    name = "nli"
    ...
    @staticmethod
    def sort_key(ex):
        return interleave_keys(len(ex.premise), len(ex.hypothesis))
...
class SNLI(NLIDataset):
    urls = ["https://nlp.stanford.edu/projects/snli/snli_1.0.zip"]
    dirname = "snli_1.0"
    name = "snli"
...
class XNLI(NLIDataset):
    urls = ["https://www.nyu.edu/projects/bowman/xnli/XNLI-1.0.zip"]
    dirname = "XNLI-1.0"
    name = "xnli"
