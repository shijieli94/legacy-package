...
from torchtext_legacy.data import BucketIterator, Dataset, Example, Field, Pipeline


class SST(Dataset):

    urls = ["https://nlp.stanford.edu/sentiment/trainDevTestTrees_PTB.zip"]
    dirname = "trees"
    name = "sst"
    ...
    def __init__(
        ...
        **kwargs
        ...
        label_field.preprocessing = Pipeline(get_label_str)
        with open(os.path.expanduser(path)) as f:
            if subtrees:
                examples = [
                    ex for line in f for ex in Example.fromtree(line, fields, True)
                ]
            else:
                examples = [Example.fromtree(line, fields) for line in f]
